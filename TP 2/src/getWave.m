function tone = getWave(number)
	...
	nsecs = 0.1;      % number of seconds of the audio file
	betweentones=0.00;
	...
	waveStar = sin(time*2*pi*1209)'+sin(time*2*pi*941)';
	wave0 = sin(time*2*pi*1336)'+sin(time*2*pi*941)';
	waveSharp = sin(time*2*pi*1447)'+sin(time*2*pi*941)';
	...
	for i = 1:size(number, 2)
		switch (number(i))
			...
			case '#'
				tone = [tone; waveSharp; nosound];
			case '*'
				tone = [tone; waveStar; nosound];
			otherwise
				'noise'
		end
	end

	wavwrite(tone,sps,'ring.wav');

	disp(['Ringing number:', num2str(number)]);
	% disp(tone);
	% wavplay(tone,sps);
end